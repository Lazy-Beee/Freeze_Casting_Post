...
def least_square_search():
    print('-----Least Square Gradient Test-----')
    x = 5e-3
    y = 0
    z = -4.5e-3
    a = gpd.GetPointData(['0.3mesh/center-250.0'], [y, z], 0.3e-3)
    active_node = a.get_active_node(x)
    cell_cent, surr_cent = a.get_neighbor_cent(x, active_node)
    print(cell_cent, surr_cent)

    print('-----Least Square Gradient Test END-----')
